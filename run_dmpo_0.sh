...
loss_type="dmpo"
diversity_reward_type="trace_jaccard" # "policy_repulsion_margin" "trace_jaccard"
num_completion_per_diversity_adapter=0 # DMPO main-only
num_completion_main_adapter=10 # DMPO G=10
num_diversity_adapters=0 # DMPO main-only
...
dmpo_base_loss_type="grpo"
dmpo_beta=1.0
dmpo_temperature=0.06666666666666667
dmpo_skip_zero_advantage_groups=False
dmpo_candidate_scope="main_only"
dmpo_sanity_check=False
...
case "$diversity_reward_type" in
    ...
    *)
        ...
        echo "Supported values: trace_jaccard, trace_jaccard3, policy_repulsion_margin, policy_repulsion_margin_barrier, external, one_minus_bleu, one_minus_bleu_score, 1-bleu"
        exit 1
        ;;
esac
...
experiment_name="DMPO_singleSource_G10_beta1_temp0.0667_${MODEL_ABB}_lr_${learning_rate}-$(date +%m%d_%H%M%S)"
...
"$PYTHON" -m accelerate.commands.launch --num_processes $num_processes \
    ...
    --loss_type ${loss_type} \
    --dmpo_base_loss_type ${dmpo_base_loss_type} \
    --dmpo_beta ${dmpo_beta} \
    --dmpo_temperature ${dmpo_temperature} \
    --dmpo_skip_zero_advantage_groups ${dmpo_skip_zero_advantage_groups} \
    --dmpo_candidate_scope ${dmpo_candidate_scope} \
    --dmpo_sanity_check ${dmpo_sanity_check} \
