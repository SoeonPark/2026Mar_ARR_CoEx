...
num_completion_per_diversity_adapter=3 # 0 3
num_completion_main_adapter=4 # 10 4
num_diversity_adapters=2 # 0 2
...
experiment_name="coex_jaccard2_short_lr_${learning_rate}-base_${num_completion_main_adapter}-${num_diversity_adapters}_div_${num_completion_per_diversity_adapter}_lT_${loss_type}-ndiv_${no_div}-ncorr_${no_correctness}-divType_${diversity_reward_type}-correctnessGated_${correctness_gated}-useIW_${use_importance_weighting}-repulsionTarget_${policy_repulsion_target}-aggregation_${diversity_aggregation}-$(date +%m%d_%H%M%S)"
